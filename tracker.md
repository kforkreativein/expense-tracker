...
| 71 | Full reset (Settings) | ✅ Completed | Danger-zone button with a confirm modal; wipes entries, transfers, subscriptions and splits on device + cloud. Wallets / types / spending categories stay |
...
| 75 | Voice orb + image/PDF import | ✅ Completed | One floating Siri-style orb, fixed just above the bottom dock on Home and every Split screen — hold to talk, double-tap for hands-free. No separate mic button or "mic ready" banner. + menu still offers photo/PDF statement → AI draft entries |
| 81 | Voice-add a split bill | ✅ Completed | Orb works from anywhere in Split — open a group first, or with exactly one active group it's picked automatically. "₹100 paid by Pratham, split equally" (or just a member's name / "split among everyone") prefills the Add Expense form — payer, amount, and equal split among the right people — for a one-tap confirm |
...
| 82 | Recurring rules retired — subscriptions do it all | ✅ Completed | Subscriptions fully replace the old recurring rules and are the only place to edit a recurring commitment. Turn on "Auto-add to ledger" (type + wallet, optional Type/pocket) and it posts a real transaction every cycle — daily/weekly/monthly/yearly — same as rent/EMI/salary did before. Leave it off and a subscription stays a pure renewal reminder, like Netflix always was. Existing recurring rules migrate to subscriptions automatically on first load (same id, so it's safe across devices) and now sync to the cloud, which recurring rules never did |
